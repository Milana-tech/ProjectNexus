...
function ReadingsPanel({ readings, loading, fetched }) {
    if (!fetched) return <Panel title="Raw Readings" accent="#2563eb" loading={false} empty emptyMsg="Enter a metric ID and fetch to load raw readings." />;
    return (
...
export default function MeasurementsView() {
    ...
    const handleFetch = async () => {
        ...
        try {
            ...
            if (!res.ok) { const b = await res.json(); throw new Error(b.detail || `Error ${res.status}`); }
